/**
 * Copied and modified code from https://github.com/jhonmart/python-format-js/blob/master/index.js
 */
/* eslint-disable */
// @ts-nocheck

export const isFormatString = (input: string) => !!input.match(/({.*?})/g)

export const format = (input: string, ...param) => {
  let count = 0,
    removeStr = [],
    ...
    simpleFormat = (mask, ...values) => {
      ...
      mask.match(/({.*?})/g).map((maskItem) => {
        let base_mask = maskItem.replace(/[}{]/g, '')

        let params =
            +base_mask.slice(1) && !base_mask.includes('.')
              ? +base_mask.slice(1)
        ...
        if (typeof params == 'number')
          ...
          let firstParam = params[1],
            fill_elem = [...'<^>.'].includes(firstParam) ? ' ' : firstParam,
          ...
          let space = !firstParam.includes('.') ? fill_elem.repeat(sz_sp) : '',
            ...
            srt_crop = values[count].match(/(\d+)([.])(\d+)/g)
              ? (+values[count]).toFixed(+params[custom_param_index])
              : values[count].slice(0, +params[custom_param_index]),
            custom_repeat = fill_elem.repeat(+params[custom_param_index]),
            count_repeat = Math.floor(
              (custom_repeat.length - values[count].length) / 2
            ),
            str_positon =
              count_repeat > 0
                ? fill_elem.repeat(count_repeat) + values[count]
                : '',
            elem_center = custom_repeat.replace(
              RegExp(`.{${str_positon.length || sz_sp}}`),
              str_positon
            )
          ...
          mask = mask.replace(
            ...
            (params.includes('>')
              ? elem
              : [...'>^.'].filter((ind) => params.includes(ind)).length < 1 ||
                +maskItem.slice(1) > 0
              ? elem.reverse()
              : params.includes('^')
              ? [elem_center]
    ...
    elemletterser = (letters, position, mask_item) => {
      ...
      if (letters_last) {
        if (letters_last.toLowerCase().includes('f')) {
        ...
        } else if ([...'dxXobn'].includes(letters_last)) {
          let op =
            +str[position] > 0
              ? mask_item.includes(' ')
                ? ' '
          ...
          outValue =
            op +
            (mask_item.includes('#') ? pad.mask[letters_last] : '') +
          ...
          outValue =
            letters_last == 'G' ? str[position].toUpperCase() : str[position]
        } else {
        ...
        outValue = !letters[5]
          ? outValue
          : simpleFormat(mask_item_format, outValue)
  ...
  if (typeof param[0] == 'object') {
    placeholder.map(
      (ix) => (input = input.replace(ix, param[0][/{.*?(\w+)?}/.exec(ix)[1]]))
    )
    return input
  }

  let searchRefs = placeholder.map((el) => {
    let v = /{(\w+)/.exec(el)
    return v ? +v[1] : v
  })
  ...
  if (!searchRefs.filter((p) => isNaN(+p) || p == null).length) {
    searchRefs.map(
      (ix, id) => (input = input.replace(placeholder[id], param[ix]))
    )
    return input
  }
  ...
  if (paramStr.length)
    ...
    placeholder.length - refParam.length > str.length ||
    (placeholder.length - refParam.length >= str.length && refParam.length)
  )
    failRun = `IndexError: tuple index out of range`
  else {
    placeholder.map((mask_item, position) => {
      let letters = /{(\d+)?:?([+_-])?(\d+)?(\W|_)?(\d+)?([eEfFdxXobcGgn])?}/.exec(
          mask_item
        ),
        expt = /{.*?([a-zA-Z])?}/.exec(mask_item)

      if (expt && expt[1] && !letters && !['eEfFdxXobcGg'].includes(expt)) {
        failRun = `ValueError: Unknown format code '${
          expt[1]
        }' for object of type '${typeof (+str[position] || str[position])}'`
  ...
  if (failRun)
    throw new Error(
      `Traceback (most recent call last):\n\t"${input}".format(${param
        .map((el_at) => (typeof el_at == 'string' ? `"${el_at}"` : el_at))
        .join(', ')})\n` + failRun
    )
  else {
